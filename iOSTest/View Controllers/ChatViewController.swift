...
*/
        ...
        chatClient.fetchChatData({ (stRspChat) in
            if stRspChat == nil { return }
            for stChatMessage in stRspChat!.data! {
                let message = Message.init()
                message.userID = stChatMessage.user_id ?? ""
                message.username = stChatMessage.name ?? ""
                message.avatarURL = URL(string: stChatMessage.avatar_url ?? "")
                message.text = stChatMessage.message ?? ""
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        let frameText = NSString(string: messages![indexPath.row].text).boundingRect(
            with: CGSize(width: tableView.frame.width - 121, height: .infinity),
            options: [.usesFontLeading, .usesLineFragmentOrigin],
            attributes: [.font: UIFont.systemFont(ofSize: 15)],
            context: nil
        )
        return frameText.height + 65
    }
